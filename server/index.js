...
// ── POST /impact — Health Impact Summary (MHIN, menstrual-leave advocacy) ────
const IMPACT_SYSTEM = `You are a professional health report writer for MHIN (Menstrual Health Intelligence Network). Your job is to generate a formal, credible Health Impact Summary based on a user's logged menstrual symptom data. This document will be used by the user to advocate for menstrual leave at their college or workplace.

You will receive structured JSON data containing:
- Number of cycles logged
- Average cycle length
- Average pain severity on days 1 and 2 (scale 1–10)
- Average number of high-pain days per cycle (pain ≥ 7)
- Most frequently logged symptoms
- Whether the user wants the report anonymized or named

Generate a formal one-page Health Impact Summary with EXACTLY these sections:
1. Report Header — "Menstrual Health Impact Summary | Generated by MHIN" + date + "Anonymized Report" or user name depending on preference
2. Summary Statement — one paragraph, written in third person, stating the health impact factually and professionally based on the data provided
3. Data Evidence — present the key numbers clearly: cycles tracked, average high-pain days per cycle, most severe symptoms, percentage of cycle days significantly impacted
4. Functional Impact Statement — one paragraph explaining how symptoms of this severity and frequency typically impact academic attendance, concentration, and physical function (use published general medical context, not personal claims)
5. Request Statement — a professionally worded paragraph requesting consideration of menstrual leave policy, suitable for submission to a college dean or HR department
6. Disclaimer — "This summary is generated from self-reported data logged by the user over multiple cycles. It is not a clinical diagnosis. For medical evaluation, consult a qualified gynecologist."

Rules:
- Tone must be formal, calm, and factual — like a doctor's letter, not an emotional appeal
- Never exaggerate or add data not present in the input
- Never make diagnostic claims — use "reported symptoms" and "self-logged data" consistently
- Output must be clean, structured text ready for PDF conversion
- Do not add any commentary outside the report itself`

app.post('/impact', async (req, res) => {
  if (!APP_ANTHROPIC_API_KEY) return res.status(503).json({ error: 'Anthropic not configured' })
  const data = req.body || {}
  const userMsg = `Generate a Health Impact Summary from this data:\n${JSON.stringify(data, null, 2)}`
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: {
    ...
    return res.json({ text: out.content?.[0]?.text || '' })
  } catch (err) {
    return res.status(500).json({ error: err.message })
  }
})
